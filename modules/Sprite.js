    ...
    draw() {
        ...
        let drawImage = image => {
            ...
            if (this.direction === 'right') {
                context.drawImage(image, 0, 0, width, height, this.relX, this.y, width * this.scale, height * this.scale)
            } else {
                const tranlateX = Game.width - this.width - this.relX
                context.drawFlip(Game.width, () => {
                    // 绘制图片的数据要用图片属性
                    // 因为粒子精灵是无宽度和高度的，绘制出来的图片它与自身宽高和精灵的scale有关
                    context.drawImage(image, 0, 0, width, height, tranlateX, this.y, width * this.scale, height * this.scale)
                })
            }
        }
        // 绘制动画
        let drawAnimation = (image, options) => {
            ...
            if (!options.flip && direction === 'right' || options.flip && direction === 'left') {
                context.drawImage(image, options.currFrame * width, 0, width, height, relX, y, width * scale, height * scale)
            } else {
                ...
                context.drawFlip(Game.width, () => {
                    ...
                    context.drawImage(image, options.currFrame * width, 0, width, height, tranlateX, y, width * scale, height * scale)
                })
            }
        }
