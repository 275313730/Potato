    ...
    draw() {
        ...
        let drawImage = img => {
            ...
            if (this.direction === 'right') {
                context.drawImage(img, 0, 0, width, height, relX + offsetLeft, y + offsetTop, width * scale, height * scale)
            } else {
                const tranlateX = Game.width - this.width - relX
                context.drawFlip(Game.width, () => {
                    // 绘制图片的数据要用图片属性
                    // 因为粒子精灵是无宽度和高度的，绘制出来的图片它与自身宽高和精灵的scale有关
                    context.drawImage(img, 0, 0, width, height, tranlateX + offsetLeft, y + offsetTop, width * scale, height * scale)
                })
            }
        }
        // 绘制动画
        let drawAnimation = (img, options) => {
            // 获取绘制数据
            const context = Game.context
            const relX = this.relX
            const y = this.y
            ...
            if (!options.flip && direction === 'right' || options.flip && direction === 'left') {
                context.drawImage(img, currFrame * width, 0, width, height, relX + offsetLeft, y + offsetTop, width * scale, height * scale)
            } else {
                ...
                context.drawFlip(Game.width, () => {
                    ...
                    context.drawImage(img, currFrame * width, 0, width, height, tranlateX + offsetLeft, y + offsetTop, width * scale, height * scale)
                })
            }
        }
